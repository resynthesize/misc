...
;; Time-stamp: <23 June 2004>
;; URL: http://centaur.maths.qmul.ac.uk/Emacs/
;; Version: $Id: csv-mode.el,v 1.41 2004/06/23 21:43:02 fjw Exp $
...
;; `csv-transpose' interchanges rows and columns.  For details, see
...
;; Note: See also csv.el by Ulf Jasper <ulf.jasper at web.de>.  It
...
;; Optionally right-align numbers.
...
(defcustom csv-align-padding 1
  "Aligned column spacing -- must be a positive integer.
Number of spaces used by `csv-align-fields' to separate columns."
  :group 'CSV
  :type 'integer)
...
(defcustom csv-invisibility-default nil
  "If non-nil, make separators in soft-aligned records invisible."
  :group 'CSV
  :type 'boolean)
...
(defun csv-toggle-invisibility ()
  ...
  (message "Separators in soft-aligned records are %svisible"
...
(easy-menu-define
  ...
  '("CSV"
    ...
    ["Make Separators Invisible" csv-toggle-invisibility :active t
     ...
     :help "If selected, separators in soft-aligned records are invisible"]
...
If not set, the region defaults to the lines of CSV data around point.
...
(defvar csv-killed-fields nil
  "The collection of fields last killed by `csv-kill-fields'.")
  ...
  (while (not (eobp))
	...
	(let ((fields fields) killed-fields field)
	  ...
	  (setq csv-killed-fields
		(cons (mapconcat 'identity killed-fields (car csv-separators))
		      csv-killed-fields))))
    (forward-line)))
...
(defun csv-end-of-field-width ()
  "Return char count to end of current field or nil if in last field.
Move to start of next field or to eol if last field."
  (let ((here (point)))
    (csv-end-of-field)
    (if (eolp) nil
      (prog1
...
(defun csv-align-fields (hard beg end)
  "Align the beginning of each field in the region to form columns.
...
BEG and END specify the region to align."
  ...
  ;; (if hard (barf-if-buffer-read-only))
  (setq end (set-marker (make-marker) end))
  (csv-unalign-fields hard beg end)
  (save-excursion
    (save-restriction
      (narrow-to-region beg end)
      (set-marker end nil)
      (goto-char (point-min))
      ;; Find width of each column but last as a list:
      (let (widths)
	(while (not (eobp))
	  (or (csv-not-looking-at-record)
	      (let ((w widths) x)
		(while (setq x (csv-end-of-field-width))
		  (if w
	...
	;; Align fields:
	(goto-char (point-min))
	(if (> csv-align-padding 0)
	    (setq widths
		  (mapcar (lambda (x) (+ csv-align-padding x)) widths)))
	(while (not (eobp))
	  (or (csv-not-looking-at-record)
	      (let ((w widths) x)
		(while (and w (setq x (csv-end-of-field-width)))
		  (setq x (make-string (- (car w) x) ?\ ))
		  ;; non-empty string only if (- (car w) x) positive
		  (if hard
		      ;; Insert spaces after separator:
		      (insert x)
		    ;; Display spaces after separator:
		    (let ((overlay (make-overlay (1- (point)) (point))))
		      (overlay-put overlay 'after-string x)
		      ;; and make separator (potentially) invisible:
...
Modifies the match data; use `save-match-data' if necessary."
  ...
  (let ((rexp (or separators "[ \f\t\n\r\v]+"))
    ...
    (while (and (string-match rexp string
	  ...
	  (setq list
		(cons (substring string start (match-beginning subexp))
		      list)))
